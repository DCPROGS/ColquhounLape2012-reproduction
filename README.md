...
The notebook was drafted with the assistance of a large language model, and the
checking that entailed is described in
[HOW_THIS_WAS_MADE.md](HOW_THIS_WAS_MADE.md). Nothing here rests on the model
having been careful: it rests on the checks below, which anyone can re-run.
...
Separately, the notebook says in section 2.2 that the two concentration-jump
APIs — the procedural one in the pinned release, and the one in later releases —
agree to better than one part in 10⁶ on everything reported here. The scripts in
[`checks/`](checks/README.md) are how that is verified: they run the whole
calculation set against two SCALCS versions and diff the results.
...
| `HOW_THIS_WAS_MADE.md` | what was generated by a language model, what prompting was used, and what checking was needed |
| `checks/` | scripts that compare the whole calculation set across two SCALCS versions, supporting the agreement claim in section 2.2 — see [checks/README.md](checks/README.md) |
